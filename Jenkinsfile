pipeline {
    ...
    stages {
        stage('Build Dev stack') {
            steps {
            sh label: '', script: 'sh "aws cloudformation create-stack --stack-name dev-mediawiki --template-body file://mediawiki-deployment.yaml --parameters file://dev-parameters.json --region \'us-east-1\'"'
              }
             }
